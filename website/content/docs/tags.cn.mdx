...
标签（也称"标记"）让你可以在项目目录之外对会话进行分类。Prime Agent Session Manager 支持带父子关系的层级标签系统。
...
<Callout type="info">
标签存储在 SQLite 数据库（`~/.pi/agent/sessions/sessions.db`）中，而非 JSONL 会话文件中。这意味着标签仅存在于你的 Prime Agent Session Manager 实例本地。
</Callout>
