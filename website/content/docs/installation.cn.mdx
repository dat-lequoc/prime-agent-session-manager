---
title: 安装指南
description: 在 macOS、Windows、Linux 上安装 Prime Agent Session Manager，或部署为无头 CLI 服务器。
---
...
打开 [http://127.0.0.1:52131/#/projects](http://127.0.0.1:52131/#/projects)。

## 桌面应用

从 [GitHub Releases](https://github.com/dat-lequoc/prime-agent-session-manager/releases) 下载最新版本。
...
打开 `.dmg` 文件，将 Prime Agent Session Manager 拖入应用程序文件夹。首次启动时，可能需要右键点击 → 打开以绕过 Gatekeeper。

### Windows

| 格式 | 文件 |
|------|------|
| 安装程序（推荐） | `Prime.Agent.Session.Manager_*_x64-setup.exe` |
...
从 [Releases](https://github.com/dat-lequoc/prime-agent-session-manager/releases) 页面下载 `pi-session-cli`（与桌面版一同发布）。
...
这将在 `http://127.0.0.1:52131` 启动 HTTP 服务器，包含嵌入式前端、WebSocket（`/ws`）和 API（`/api`）。
